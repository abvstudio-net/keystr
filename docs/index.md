# nos2x-fox — Firefox Nostr add-on

![nos2x-fox logo](images/logo.png){ width="128" }

**nos2x-fox** is a Firefox add-on that lets you manage your Nostr keys
in one place, and then sign events in websites without exposing your private key.

It also has some additional features like giving permissions to sites for a
limited period of time, or managing and saving your preferred relays.

More information at the
[NIP-07 documentation](https://github.com/nostr-protocol/nips/blob/master/07.md).

## Important links

- [Add-on in Firefox AMO](https://addons.mozilla.org/en-US/firefox/addon/nos2x-fox/) —
  the add-on directly on the Firefox marketplace.
- [Releases](https://github.com/abvstudio-net/nos2x-fox/releases) —
  you'll find the `.xpi` files to install locally (signed by Mozilla).
- [Main GitHub repo](https://github.com/abvstudio-net/nos2x-fox) —
  screenshots, development info, and the code of course.

## License and Credits

- LICENSE: MIT. Original work by [fiatjaf](https://github.com/fiatjaf),
  Firefox adaptation by [diegogurpegui](https://github.com/diegogurpegui).
- Design taken from [Flydexo](https://github.com/Flydexo). See
  [fiatjaf/nos2x#15](https://github.com/fiatjaf/nos2x/pull/15).
- Icons from [IonIcons](https://ionic.io/ionicons).
